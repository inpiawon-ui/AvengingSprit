...
⚠ **가운데로 모으지 마라.** 태블릿 4:3 은 보이는 캔버스가 960 폭이라, 720 에 가둬
  ...
  `ScreenFit` 이 화면 끝까지 늘리게 한다. 판 안의 칸(상자 3칸 · 하단 3버튼)은
  `ScreenFitShare` 로 남는 폭을 그린 비율대로 나눠 갖는다 — 칸 사이 간격은 그린
  값을 지키므로 줄 모양 그대로 폭만 커진다.
  붙이는 곳: `ScreenFitInstaller.Shares`

  모드 카드 3장은 기운 낱장이라 폭을 못 늘린다. `ScreenFitZoom` 으로 **통째로
  키운다** — 카드도 간격도 같은 배율로 커진다. 붙이는 곳: `ScreenFitInstaller.Zooms`
...
#   가운데 카드가 중앙을 벗어난다. 카드 세 장 + 화살표는 `ScreenFitZoom` 이 통째로
#   키운다(폭을 늘리면 기운 테가 뭉개진다).
#
# ⚠ 판 높이 549 는 **세로 예산**이다. 상자 띠 밑(896)부터 하단 바 그림이 실제로
#   보이기 시작하는 곳(1383+62=1445)까지 — 배율이 이 높이를 넘지 않게 묶인다.
#   카드(480)에 딱 맞춰 492 로 잡아 두었더니 4:3 에서 배율이 1.02 로 묶여
#   카드 사이 구멍이 그대로 남았다(2026-09-16).
add('GameModeGroup', (0, 896, MOCK_W, 549))
