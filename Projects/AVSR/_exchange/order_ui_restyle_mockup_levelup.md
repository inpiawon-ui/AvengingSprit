# 발주 — 인게임 UI 퀄업 시안: 레벨업 카드 선택 (완성 화면 720 × 1280 한 장)
...
| 파일 | 무엇 |
|---|---|
| `Projects/AVSR/_exchange/ref/ui_now_levelup.png` | **이 화면의 지금 모습** (720 × 1280) — 레이아웃의 정본. 생김새는 따르지 마라 |
...
- 무엇이 있나: 적을 잡아 레벨이 오르면 뜬다. 위에 LEVEL UP 제목과 안내 한 줄, 가운데 카드 3장(각 카드: 등급 딱지 RARE/COMMON · 아이콘 · 이름 · 효과 설명). 하나를 누르면 닫힌다
- 중요한 것: 카드 3장이 한눈에 비교돼야 한다. 등급 차이가 보여야 한다
...
| 항목 | 값 |
|---|---|
| 파일 | `ui_mockup_levelup.png` |
| 크기 | **720 × 1280 px** 완성 화면 한 장 |
| 뒤 배경 | 통과한 새 전투 화면(쓰레기 집적장)이 깔린 상태로 그린다 |
| 레이아웃 | `ui_now_levelup.png` 와 **같은 자리 · 같은 구성** |
| 글자 | `ui_now_levelup.png` 에 적힌 글자를 **그대로** 옮겨 적는다(일본어). 임의로 바꾸지 않는다 |
...
`Projects/AVSR/_exchange/in/ui_mockup_levelup.png`
