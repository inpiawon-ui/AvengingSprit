# 발주 — 인게임 UI 조각: 왼쪽 아래 **이동 조이스틱 판**(방향 화살표 포함) (120 × 120)

## ⚠ 재발주 — 1차에 **가운데 빨간 공(손잡이)이 판에 박혀** 왔다

손잡이는 hud_dpad_knob 으로 **따로 받아 판 위에서 움직인다.** 판에 그려 두면 빨간 공이 두 개 보인다.
**이번에는 판과 방향 화살표만. 가운데는 손잡이가 올라갈 빈 자리로 비워 둔다.**
...
| `Assets/BaseResource/InGameMainUI/hud_dpad_base.png` | 지금 이 자리의 그림(교체 대상) — 크기 · 쓰임만 참고, 생김새는 따르지 마라 |

## 이 조각

- 무엇: 왼쪽 아래 **이동 조이스틱 판**(방향 화살표 포함)
...
`Projects/AVSR/_exchange/in/uipiece_hud_dpad_base.png`
