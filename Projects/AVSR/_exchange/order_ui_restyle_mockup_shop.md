# 발주 — 인게임 UI 퀄업 시안: 상점 「유령 노점」 (완성 화면 720 × 1280 한 장)
...
| 파일 | 무엇 |
|---|---|
| `Projects/AVSR/_exchange/ref/ui_now_shop.png` | **이 화면의 지금 모습** (720 × 1280) — 레이아웃의 정본. 생김새는 따르지 마라 |
...
- 무엇이 있나: 방에 선 노점을 건드리면 뜬다. 제목 · 보유 골드 · 남은 구매 횟수 한 줄, 진열 6줄(각 줄: 왼쪽 아이콘 · 이름 · 작은 설명 · 오른쪽 값), 아래 「나간다」 버튼. 상인은 **유령 상인**이고 방에 서 있는 노점은 보라 차양을 친 나무 가판이다
- 중요한 것: 여섯 줄의 값이 한눈에 비교돼야 한다. 못 사는 줄은 흐려진다
...
| 항목 | 값 |
|---|---|
| 파일 | `ui_mockup_shop.png` |
| 크기 | **720 × 1280 px** 완성 화면 한 장 |
| 뒤 배경 | 통과한 새 전투 화면(쓰레기 집적장)이 깔린 상태로 그린다 |
| 레이아웃 | `ui_now_shop.png` 와 **같은 자리 · 같은 구성** |
| 글자 | `ui_now_shop.png` 에 적힌 글자를 **그대로** 옮겨 적는다(일본어). 임의로 바꾸지 않는다 |
...
`Projects/AVSR/_exchange/in/ui_mockup_shop.png`
