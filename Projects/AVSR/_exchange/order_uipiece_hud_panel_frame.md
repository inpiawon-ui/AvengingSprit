# 발주 — 인게임 UI 조각: 상단 HUD 의 **큰 판 테두리 (120 × 120)
...
- 무엇: 상단 HUD 의 **큰 판 테두리** — PLAYER SOUL 판(374×94) · RUN RESOURCES 판(252×94) · CHAPTER 판(326×118) · 호스트 판(374×118) 에 **늘려서** 같이 쓴다. 시안 속 판들의 테두리 · 모서리 · 안쪽 면을 한 장으로
...
| 항목 | 값 |
|---|---|
| 파일 | `uipiece_hud_panel_frame.png` |
| 크기 | **120 × 120 px 정확히** |
| 형식 | PNG **투명 배경**(알파). 조각 바깥은 완전히 투명. 뒤에 전투 화면을 그려 넣지 마라 |
| 글자 | **글자 · 숫자 금지** — 글자는 게임이 쓴다 |
| 늘려 쓰기 | 이 조각은 게임이 **늘려서** 쓴다(9-슬라이스). 테두리 · 모서리 장식은 **네 변에서 24 px 안쪽에만**. 가운데는 늘어나도 티가 안 나는 고른 면 |
...
`Projects/AVSR/_exchange/in/uipiece_hud_panel_frame.png`
