# 발주 — 인게임 UI 조각: HUD 안의 **작은 칸 테두리 (48 × 48)
...
- 무엇: HUD 안의 **작은 칸 테두리** — 골드 칸(114×52) · 젬 칸(112×52) · 레벨 배지(96×30) 에 **늘려서** 쓴다
...
| 항목 | 값 |
|---|---|
| 파일 | `uipiece_hud_cell_frame.png` |
| 크기 | **48 × 48 px 정확히** |
| 형식 | PNG **투명 배경**(알파). 조각 바깥은 완전히 투명. 뒤에 전투 화면을 그려 넣지 마라 |
| 글자 | **글자 · 숫자 금지** — 글자는 게임이 쓴다 |
| 늘려 쓰기 | 이 조각은 게임이 **늘려서** 쓴다(9-슬라이스). 테두리는 **네 변에서 12 px 안쪽에만**. 가운데는 고른 면 |
...
`Projects/AVSR/_exchange/in/uipiece_hud_cell_frame.png`
