# 발주 — 인게임 UI 퀄업 시안: 전투 HUD (완성 화면 720 × 1280 한 장)
...
| 파일 | 무엇 |
|---|---|
| `Projects/AVSR/_exchange/ref/ui_now_hud.png` | **이 화면의 지금 모습** (720 × 1280) — 레이아웃의 정본. 생김새는 따르지 마라 |
...
- 무엇이 있나: 전투 중 늘 떠 있는 것 전부 — 위쪽 상단 바(유령 초상 · 이름 · HP 바 · 레벨 · 골드 · 젬 · 일시정지), 그 아래 챕터 판(CHAPTER 1 · 무대 이름 · ROOM 01 / 10 · 진행 막대), 아래쪽 왼편 이동 조이스틱, 아래쪽 오른편 빙의(POSSESS) 버튼
- 중요한 것: 전투를 **가리지 않는 것**이 제일 중요하다. 늘 떠 있으니 가장 조용해야 한다
...
| 항목 | 값 |
|---|---|
| 파일 | `ui_mockup_hud.png` |
| 크기 | **720 × 1280 px** 완성 화면 한 장 |
| 뒤 배경 | 통과한 새 전투 화면(쓰레기 집적장)이 깔린 상태로 그린다 |
| 레이아웃 | `ui_now_hud.png` 와 **같은 자리 · 같은 구성** |
| 글자 | `ui_now_hud.png` 에 적힌 글자를 **그대로** 옮겨 적는다(일본어). 임의로 바꾸지 않는다 |
...
`Projects/AVSR/_exchange/in/ui_mockup_hud.png`
