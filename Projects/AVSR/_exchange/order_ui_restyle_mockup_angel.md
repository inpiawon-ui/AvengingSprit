# 발주 — 인게임 UI 퀄업 시안: 천사 이벤트 「회복의 제단」 (완성 화면 720 × 1280 한 장)
...
| 파일 | 무엇 |
|---|---|
| `Projects/AVSR/_exchange/ref/ui_now_angel.png` | **이 화면의 지금 모습** (720 × 1280) — 레이아웃의 정본. 생김새는 따르지 마라 |
...
- 무엇이 있나: 공짜로 셋 중 하나를 받는 창(거절 버튼 없음). 제목 판, 안내 한 줄, 선택 칸 3줄(각 칸: 왼쪽 아이콘 · 이름 · 작은 설명). 천사 · 축복의 기운
- 중요한 것: 세 칸이 고르게 눌러질 것처럼 보여야 한다
...
| 항목 | 값 |
|---|---|
| 파일 | `ui_mockup_angel.png` |
| 크기 | **720 × 1280 px** 완성 화면 한 장 |
| 뒤 배경 | 통과한 새 전투 화면(쓰레기 집적장)이 깔린 상태로 그린다 |
| 레이아웃 | `ui_now_angel.png` 와 **같은 자리 · 같은 구성** |
| 글자 | `ui_now_angel.png` 에 적힌 글자를 **그대로** 옮겨 적는다(일본어). 임의로 바꾸지 않는다 |
...
`Projects/AVSR/_exchange/in/ui_mockup_angel.png`
