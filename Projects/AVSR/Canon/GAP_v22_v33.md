...
| 항목 | 결정 |
|---|---|
| 카드 정본 | **v2.3** (`CARD_BUILD_EVOLUTION_MASTER_v2.3_CHAPTER_SYNC.xlsx`) |
| 방 규격 | **정본 폭 ÷ 4, 높이는 정본 그대로** |
| 레벨 디자인 정본 | v3.3 FINAL LOCK (48방) |

### v2.3 = v2.2 + 챕터 통합 시트 하나

카드 32종의 정의·수치·레벨값은 **v2.2와 완전히 동일**하다(전수 대조 확인).
v2.3 이 더한 것은 `44_CHAPTER_INTEGRATION` 시트뿐이고, 챕터별 획득 페이싱이 들어 있다.

| 챕터 | 레벨업 선택 | 엘리트 선택 | 이벤트 확률 | 상점 제시 | Evolution 평균 신규 |
|---|---|---|---|---|---|
| CH1 | 4회 | 1 | 0.4 | 3 | 0.42 |
| CH2 | 5회 | 1.4 | 0.6 | 3 | 0.91 |
| CH3 | 6회 | 1.8 | 0.7 | 3 | 1.34 |

> 붙여 주신 카드 도감 이미지는 v1.0 이라 C017~C020·C028 의 이름·효과가 다르고
> 등급 분포도 다르다(11/10/7/4 → 8/12/8/4). **이미지는 폐기하고 zip 안의
> `VISUAL_C/D_CARD_ENCYCLOPEDIA` 를 쓴다.**

### 방 폭 ÷ 4 — 성립한다

**세로 카메라 추적이 이미 구현돼 있다** (`BattleDirector.ApplyScroll`, 창은 고정하고 방을 민다).
그래서 방이 화면보다 세로로 길어도 그대로 굴러간다.

| 정본 | ÷4 적용 | 화면(9:16) 몇 개 높이 |
|---|---|---|
| 24 × 14 | **6 × 14** | 1.31 |
| 28 × 14 | **7 × 14** | 1.13 |
| 32 × 14 | **8 × 14** | 1.00 |
...
지금 방(0.94 화면)과 비슷하거나 최대 1.6배 길어지는 정도다. 카메라 정책 변경 없이 간다.
...
| H08 | DRAGON — GREEN | `salamander` |
| H09 | DRAGON — RED | `dragoon` |
...
**해야 할 일**
- 신규 2종: H06 미사일 코만도 · H23 사신 — 둘 다 원작 시트가 **있다**
  (`Enemies - Commando (Missiles Pack)__sheet.png` / `Miscellaneous - Death__sheet.png`)
- 히든 2종(H24/H25)은 CH1~3 에서 제외 — 지금은 손대지 않는다

> ⚠ 2026-08-20 정정 — 앞 판에서 `salamander`/`dragoon` 을 **Red Dragon 중복**이라 적고
> 신규를 3종으로 셌다. 실제로는 `salamander` = Dragon Green(H08),
> `dragoon` = Dragon Red(H09) 로 **이미 1:1 대응돼 있다.** 그림이 필요한 것은 2종뿐이다.
...
| 7 | **호스트 21 → 23 (H06·H23)** | 작음 | 캐릭터 2종 × 5방향 |
...
| 워크오더 | 내용 | 장수 |
|---|---|---|
| WO-A1 | H23 DEATH 5방향 × 8동작 | 40 |
| WO-A2 | H06 COMMANDO MISSILE 5방향 × 8동작 | 40 |
