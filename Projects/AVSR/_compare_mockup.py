...
목업 호스트 목록은 3열 5행 = 15칸이고 우리 테이블도 15종이다.
람보(레이저)·마법사(녹색)·닌자(적색)은 색 변형이 아니라 별개 캐릭터다 —
인물·복장·무기가 다르다(람보는 갈색머리 올리브 vs 금발 청색 바이저).
"""
import colorsys
import os
...
SLOTS = [
    ...
    (0, 2, 'rambo_laser', '람보(레이저)'),
    (1, 0, 'wizard', '마법사(분홍)'), (1, 1, 'wizard_green', '마법사(녹색)'),
    (1, 2, 'ninja', '닌자(청색)'),
    (2, 0, 'ninja_red', '닌자(적색)'), (2, 1, 'mafia', '마피아'),
    (2, 2, 'hitman', '히트맨'),
