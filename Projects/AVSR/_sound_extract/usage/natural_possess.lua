...
local POSSESS_KIND = tonumber(os.getenv("AVS_POSSESS_KIND") or "-1")
if INVINCIBLE == "1" then
    host_energy_tap = main:install_write_tap(HOST_OBJ + 0x66, HOST_OBJ + 0x67, "inv_host", function(offset, data, mask)
        if POSSESS_KIND > 0 and main:read_u16(0x7a51e) ~= POSSESS_KIND then return data end
...
if TELEPORT then
    ...
    tele_sub = emu.add_machine_frame_notifier(function()
        ...
        nextTeleport = t + 2.0
        local nowHost = main:read_u16(0x7a51e)
        if nowHost == 0 or (POSSESS_KIND > 0 and nowHost ~= POSSESS_KIND) then return end
...
-- 자연 빙의: 유령 상태면 목표 종류 적 위로 유령을 옮겨 닿게 한다. 다른 종류에 빙의하면 호스트 에너지를 0으로 만들어 버리게 한다.
local GHOST_OBJ = 0x79e5a
if POSSESS_KIND > 0 then
    local nextGhostMove = 5
    possess_sub = emu.add_machine_frame_notifier(function()
        local t = emu.time()
        if t < nextGhostMove then return end
        nextGhostMove = t + 1.0
        local host = main:read_u16(0x7a51e)
        if host == POSSESS_KIND then return end
        if host ~= 0 then
            main:write_u16(HOST_OBJ + 0x66, 0)
            log:write(string.format("K %.1f drop host=%d\n", t, host))
            log:flush()
            return
        end
        for _, arr in ipairs(OBJECT_ARRAYS) do
            for i = 0, arr[2] - 1 do
                local o = arr[1] + i * OBJ_SIZE
                if (main:read_u16(o) & 0x8000) ~= 0 and main:read_u16(o + 0x10) == POSSESS_KIND then
                    main:write_u32(GHOST_OBJ + 0x28, main:read_u32(o + 0x28))
                    main:write_u32(GHOST_OBJ + 0x32, main:read_u32(o + 0x32))
                    log:write(string.format("G %.1f ghost->kind=%d obj=%06x\n", t, POSSESS_KIND, o))
                    log:flush()
                    return
                end
            end
