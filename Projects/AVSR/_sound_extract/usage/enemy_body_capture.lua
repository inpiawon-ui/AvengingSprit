...
-- 적 몸체 오려내기: 적 하나를 찍고(A), X를 64픽셀 옮긴 다음 프레임을 찍은(B) 뒤 원위치한다.
-- A/B 차이가 곧 그 적의 스프라이트다. 종류값은 객체에서 직접 읽는다.
local IDENT = os.getenv("AVS_IDENT") == "1"
local MOVE_X = 0x400000
if IDENT then
    local nextShot, phase, target, savedX = 20, 0, nil, 0
    ident_sub = emu.add_machine_frame_notifier(function()
        local t = emu.time()
        if phase == 0 then
            if t < nextShot or shots >= MAX_SHOTS - 2 then return end
            local candidates = {}
            for _, arr in ipairs(OBJECT_ARRAYS) do
                for i = 0, arr[2] - 1 do
                    local o = arr[1] + i * OBJ_SIZE
                    local kind = main:read_u16(o + 0x10)
                    if (main:read_u16(o) & 0x8000) ~= 0 and kind >= 1 and kind <= 26 then candidates[#candidates + 1] = o end
                end
            end
            if #candidates == 0 then nextShot = t + 1; return end
            target = candidates[math.random(#candidates)]
            manager.machine.video:snapshot()
            local shotA = shots
            shots = shots + 1
            savedX = main:read_u32(target + 0x28)
            main:write_u32(target + 0x28, (savedX + MOVE_X) & 0xffffffff)
            log:write(string.format("I %.2f kind=%d obj=%06x shotA=%d\n", t, main:read_u16(target + 0x10), target, shotA))
            phase = 1
        else
            manager.machine.video:snapshot()
            local shotB = shots
            shots = shots + 1
            main:write_u32(target + 0x28, savedX)
            log:write(string.format("J shotB=%d\n", shotB))
            log:flush()
            phase, nextShot = 0, t + 2.5
        end
    end)
end