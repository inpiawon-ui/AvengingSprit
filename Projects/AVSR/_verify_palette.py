...
# 오브·보석 같은 작은 포인트는 30~40px(2%대)면 충분히 보인다. 3% 로 잡았더니
# 파랗게 칠한 오브를 반려했다. 목업 쪽 비율은 카드 배경이 섞여 부풀어 있으므로
# "있느냐 없느냐"만 본다.
FAM_ALIVE = 1.5
...
def spectrum(im, alpha):
    ...
    ⚠ 명도 하한은 **목업 크롭에만** 건다. 목업은 어두운 카드 위에 그려져 있어
      배경을 걸러야 하지만, 우리 스프라이트는 투명 배경이라 알파가 이미
      배경을 뺀다. 여기에 같은 하한을 걸면 **검은 옷이 통째로 사라진다** —
      흡혈귀의 검은 정장이 픽셀의 절반인데 무채색 8% 로 나와 반려됐다.
      검정도 무채색이다.
    """
    px = im.load()
    w, h = im.size
    f = {}
    grey = tot = 0
    dark_floor = 0.0 if alpha else 0.18
    for y in range(h):
        for x in range(w):
            c = px[x, y]
            if alpha and c[3] <= 8:
