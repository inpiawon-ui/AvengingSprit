...
승인된 목업 편차 (AVSR_Decisions.md)
  · 그리드 **15칸(5행)** — 목업 그대로. 세 칸(람보 레이저·마법사 녹색·닌자 적색)은
    색 변형이 아니라 별개 캐릭터다(인물·복장·무기가 다르다).
...
# 5행: 5*150 + 4*8 = 782 → 바닥 1082. TipBar(1203) 와 121px 여유.
# 좌우 열이 갈라져 있어 오른쪽 버튼들과는 겹치지 않는다.
add('HostGrid', (mx(14), 300, 327, 782), grid=[105, 150, 6, 8, 3])
