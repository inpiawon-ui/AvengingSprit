...
namespace Game.Character
{
    ...
    public enum BossDraw
    {
        ...
        // ── B04 파이썬 — 벽 구멍에서 나온다 ─────────────────────
        HeadLunge,       // 머리 뻗기 — 나온 구멍에서 곧장 아래로 목을 뻗는다
        VenomCloud,      // 독 뱉기 — 웅덩이가 남아 다음을 못 피하게 한다
        BrickFall,       // 벽돌 낙하 — 벽이 부서져 세 곳에 떨어진다. 맞으면 굳는다
        BodyShove,       // 몸통 밀기 — 벽 전체가 방 안으로 밀고 들어온다
        ...
        // ── B05 로봇 스네이크 — 발밑을 뚫고 나온다 ───────────────
        BurrowStrike,    // 솟아오름 — **내 발밑**에 구멍이 생기며 솟는다. 맞으면 굳는다
        RailLaser,       // 레이저 — 조준선이 먼저 그려진다
        DebrisFall,      // 천장 파편 — 그림자 다섯
        // ⚠ 안 쓴다(기획 2026-09-07 — 고정 구멍 6개를 버렸다).
        //   자리를 빼면 아래 슬러지 넷의 저장값이 한 칸씩 밀리므로 남겨 둔다.
        Retired_FullEmergence,
    ...
    public enum MoveRange
    {
        ...
        /// <summary>
        /// **제 도형 안에 들어왔을 때만.** 문턱을 따로 적지 않고 이 행동의 반경을 쓴다.
        /// 반경을 고치면 조건도 같이 따라오므로 두 값이 어긋날 수가 없다.
        /// (「똬리」 · 「머리 물기」 — 대상이 범위에 오면 발동)
        /// </summary>
        InShape,

        /// <summary>
        /// **이 보스의 <see cref="InShape"/> 행동 반경을 전부 벗어났을 때만.**
        /// 붙어서 쓰는 것들이 아무것도 안 닿는 거리에서만 나가는 원거리 채움이다.
        /// (「마디 사출」 — 똬리·머리 물기 거리에 없을 경우)
        /// </summary>
        OutOfShapes,
    }
