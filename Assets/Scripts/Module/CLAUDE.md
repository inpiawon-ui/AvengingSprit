...
| `TryClaim(slot, out ChestReward)` | 완료된 칸을 열어 보상 지급 후 칸을 비운다. 보상은 골드 + 호스트별 조각 목록 |

이벤트: `ChestChangedEvent`(칸 상태가 바뀜) · `ChestGrantedEvent{GrantedChestKey, Slot, Accepted}` · `ChestOpenedEvent{OpenedChestKey, RewardGold, RewardShardHostKeys[], RewardShardCounts[]}`
...
- **칸이 다 차면 상자를 안 준다.** 클리어 때 빈 칸이 없으면 `TryGrant` 가 `false` 를 돌려주고, 인게임 결과 화면이 「칸이 없어 상자를 못 받았다」를 알린다. **골드는 그래도 준다.** 조용히 버리면 왜 안 들어왔는지 알 길이 없다.
- **보상은 열 때 굴린다.** 받을 때 굴려 저장하면 저장 파일을 들여다보고 마음에 안 들면 다시 받는 식이 가능해진다.
- **상자는 챕터를 깼을 때만 나온다**(기획 2026-09-18). 죽으면 골드도 상자도 없다. 한 판 = 한 챕터.
  등급은 챕터로 정한다 — 1·2 은(`silver`) · 3·4 금(`gold`) · 5·6 백금(`platinum`). 깬 챕터를 다시 깨도 준다.
- **보상은 골드와 호스트 조각뿐이다.** 조각은 호스트 여러 명에게 나눠 준다 — 은 1~2명 · 금 2~3명 · 백금 3~4명.
  등급이 높은 상자일수록 S·A 호스트가 잘 나온다(표의 등급 무게 B·A·S). **해금 안 된 호스트도 준다.**
- 해제 시간은 **테스트용** 1·2·3분이다. 출시 전에 다시 정한다.
