...
{
    /// <summary>
    /// `HostTable` · `ActiveSkillTable` 에셋을 확정 데이터로 생성하고 Addressable 에 등록한다.
    ///
    /// 데이터 출처는 `Projects/AVSR/AVSR_Decisions.md` §4 (호스트 12종 · 해금 조건).
    /// 액티브 스킬 정본은 제안서 로스터 페이지(slide_07).
    /// </summary>
    public static class CreateAssetTables
    {
        ...
        private static readonly object[][] Bosses =
        {
            ...
            new object[]{ "crusher",     2, "CRUSHER",    "크러셔",       "unit_crusher", 1.25f, 1.15f, 1.30f, 0.75f,
                new object[][] {
        ...
        private static readonly object[][] Buffs =
        {
            ...
            new object[]{ "ult_charge", "액티브 스킬 충전", "충전 속도 +30%", BuffKind.ActiveSkillCharge, 30, true, "#F07828", "", "", "", 1, 1.0f, true },
            ...
            new object[]{ "wide_echo", "넓은 울림", "광역 반경 +10%", BuffKind.AoeRadius, 10, true, "#4AA8E8", "BUF_U05", "AoE radius +10%", "Universal", 1, 0.8f, true },
            new object[]{ "marked_payload", "표식 탄두", "표식 대상 명중 시 릴레이 탄 +1", BuffKind.MarkPayload, 1, false, "#F0B428", "BUF_T01", "Mark consume: Relay Bullet +1", "Tag", 2, 1.0f, true },
            ...
            new object[]{ "blood_debt", "피의 부채", "흡혈 초과분이 고스트 체력으로", BuffKind.BloodDebt, 2, false, "#E04A7A", "BUF_T04", "Leech overheal to Ghost HP, room cap2", "Tag", 2, 0.8f, true },
            new object[]{ "bank_shot", "뱅크 샷", "탄이 벽에서 한 번 튕긴다", BuffKind.Ricochet, 1, false, "#C98CF0", "BUF_T05", "first bounce duplicates at 60% damage", "Tag", 2, 0.9f, true },
            new object[]{ "smart_deployment", "스마트 배치", "설치물 재조준 25% 빠르게", BuffKind.DeployRetarget, 25, true, "#9AA4B4", "BUF_T06", "deployables retarget 25% faster", "Tag", 2, 0.8f, true },
            new object[]{ "last_magazine", "마지막 탄창", "마지막 탄·마무리 +25%", BuffKind.LastShot, 25, true, "#E8604A", "BUF_A01", "last shot/finisher +25%", "AttackStyle", 1, 0.8f, true },
            ...
            new object[]{ "return_path", "귀환 경로", "되돌아오는 탄이 50% 피해", BuffKind.ReturnDamage, 50, false, "#A0E0FF", "BUF_A03", "returning projectile deals 50%", "AttackStyle", 2, 0.7f, true },
        ...
        // activeSkillKey, 영문명, 한글명, 설명
        private static readonly string[][] ActiveSkills =
        {
        ...
        /// <summary>
        /// ⚠ 이 도구는 표를 **처음 만드는** 용도다. 돌릴 때마다 표를 통째로 새로 쓴다.
        ///
        /// 지금 HostTable·ActiveSkillTable·BuffTable 은 정본 임포터가 주인이다
        /// (`Canon v2.3 임포트 (호스트·액티브 스킬)` / `(카드 32종)`).
        /// 아무 생각 없이 이것을 돌리면 호스트 23→21, 액티브 스킬 25→12, 카드 32→옛 버프 29 로
        /// **되돌아간다.** 실제로 한 번 그렇게 날렸다.
        ///
        /// 그래서 이미 정본이 구워 놓은 표가 있으면 그 표는 건드리지 않고 넘어간다.
        /// 정말 씨앗 값으로 되돌리고 싶으면 그 에셋을 지우고 다시 돌린다.
        /// </summary>
        /// <summary>
        /// 그 표가 씨앗 값보다 많은 줄을 갖고 있으면 정본이 구운 것으로 본다.
        /// 줄 수만 보는 거친 판정이지만, 여기서 필요한 것은 "덮어써도 되는가" 하나뿐이다.
        /// </summary>
        private static bool IsCanonBaked(string path, string field, int seedMax)
        {
            var so = AssetDatabase.LoadAssetAtPath<ScriptableObject>(path);
            if (so == null) return false;
            var fi = so.GetType().GetField(field,
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            if (fi == null) return false;
            return fi.GetValue(so) is System.Collections.IList list && list.Count > seedMax;
        }
        ...
        public static void Run()
        {
            ...
            // 정본이 이미 채운 표는 손대지 않는다
            bool keepHost = IsCanonBaked($"{Dir}/HostTable.asset", "_entries", 22);
            bool keepUlt  = IsCanonBaked($"{Dir}/ActiveSkillTable.asset", "_entries", 13);
            bool keepBuff = IsCanonBaked($"{Dir}/BuffTable.asset", "_entries", 30);
            if (keepHost || keepUlt || keepBuff)
                Debug.LogWarning("[CreateAssetTables] 정본이 구운 표는 건너뜁니다 — "
                    + $"Host={keepHost} ActiveSkill={keepUlt} Buff={keepBuff}. "
                    + "씨앗 값으로 되돌리려면 해당 에셋을 지우고 다시 돌리세요.");
            ...
            var entries = Hosts.Select(h =>
            {
                ...
                Set(e, "_hp", h[4]); Set(e, "_atk", h[5]); Set(e, "_spd", h[6]); Set(e, "_atkSpeed", h[7]);
                ...
                // 액티브 스킬 쿨(초). T1 8 · T2 14 · T3 20 · T4 28 (설계안 §6-4).
            ...
            if (!keepHost) SaveAsset(host, $"{Dir}/HostTable.asset");

            var ult = ScriptableObject.CreateInstance<ActiveSkillTable>();
            var uEntries = ActiveSkills.Select(u =>
            {
                var e = new ActiveSkillEntry();
                Set(e, "_activeSkillKey", u[0]); Set(e, "_nameEn", u[1]);
                Set(e, "_nameKr", u[2]); Set(e, "_description", u[3]);
                return e;
            }).ToArray();
            Set(ult, "_entries", uEntries);
            if (!keepUlt) SaveAsset(ult, $"{Dir}/ActiveSkillTable.asset");
            ...
            if (!keepBuff) SaveAsset(buff, $"{Dir}/BuffTable.asset");
            ...
            RegisterAddressable($"{Dir}/ActiveSkillTable.asset", "TableData/ActiveSkillTable");
            Debug.Log($"[CreateAssetTables] 호스트 {entries.Length}종 · 액티브 스킬 {uEntries.Length}종 생성 완료");
        }
