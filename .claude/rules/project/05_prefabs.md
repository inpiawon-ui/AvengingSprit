...
**루트의 직속 자식은 가로로 가운데(`anchorX 0.5`) 또는 스트레치(`0~1`)여야 한다.**
`anchorX 0~0` 으로 왼쪽에 못 박으면 **기준 해상도보다 넓은 화면에서 통째로 왼쪽으로 쏠린다.**

기준 해상도가 720 폭인데, contain 규칙(위 CanvasScaler 절)에 따라
태블릿에서는 **보이는 캔버스가 960 폭**이 된다.

```
폰   720×1280   보이는 폭 720   왼쪽 앵커라도 딱 맞는다
태블릿 768×1024  보이는 폭 960   왼쪽 앵커면 오른쪽에 240 이 통째로 남는다
```

> ⚠ 2026-09-01 에 실제로 그랬다. 타이틀 5개 · 로비 7개 · 인게임 7개 ·
> 호스트선택 8개가 전부 `anchorX 0~0` 이라 태블릿에서 화면이 왼쪽으로 쏠렸다.

### 고치는 법

**루트의 직속 자식만** 고치면 된다 — 그 아래는 부모를 따라 같이 움직인다.

```
anchorX  0 → 0.5
x        기존값 − 360        (= 기준 폭 720 의 절반)
sizeDelta 그대로
```

앵커를 바꾸면 Unity 가 크기를 다시 계산하므로 **`sizeDelta` 를 다시 넣어 준다.**

### 예외 — 건드리지 않는 것

- **가로 스트레치**(`anchorX 0~1`) — 배경·터치 영역. 이미 화면을 채운다
- 한쪽에 붙는 요소는 **가운데 앵커로 옮긴 뒤** 제 오프셋을 유지한다.
  로비 `ChapterCard` 는 720 안에서 왼쪽 15 였고, 960 에서 135(=120+15) 가 된다 —
  720 블록이 통째로 가운데로 오고 그 안에서 제자리를 지키는 것이 맞다
