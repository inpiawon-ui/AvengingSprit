...
시안 `Projects/AVSR/_exchange/out/30_tile/tile_concept.png` 를 재서 정한 값이다.

| 항목 | 값 |
|------|-----|
| 오브젝트 한 칸 | **72 × 72 px = 1 m** |
| 배경 바닥 타일 | **36 × 36 px** |
| 비율 | **오브젝트 1칸 = 바닥 타일 2 × 2** |
| 방 크기 | `10 × 13` 칸 = `720 × 936 px` |
| 안쪽 바닥(물건 놓는 곳) | 가로 칸 1~9 · 세로 칸 1~10.5 |

**규칙 — 예외 없다.**
1. 오브젝트 발자국은 **정수 칸**이다. 반 칸(1.5) 금지.
2. 그림 캔버스 = `가로칸 × 72` × (`세로칸 × 72` + 솟음).
   그림의 **맨 아랫줄이 접지선**이고 발자국 아래 변과 맞물린다.
3. 배경 바닥 줄눈은 36 px 주기, 방 원점(0,0) 위상. 72 를 정확히 나누므로
   오브젝트 변이 언제나 바닥 줄눈 위에 떨어진다.
4. 원근 금지 — 정사영(65° 기울인 탑다운)이라 칸은 어디서나 같은 크기다.

| 종류 | 칸 | 캔버스 px | 시안 위치 |
|------|-----|-----------|-----------|
| `BLOCK` 석조 블록 | 1 × 1 | 72 × 102 | 왼쪽 위 벽 |
| `PILLAR` 부서진 기둥 | 1 × 1 | 72 × 166 | 한가운데 |
| `CRATE` 철제 상자 | 2 × 1 | 144 × 132 | 오른쪽 아래 |
| `LOW_COVER` 파이프 다발 | 3 × 1 | 216 × 102 | 오른쪽 위 |
| `BARRICADE` 파이프 난간 | 3 × 1 | 216 × 102 | 왼쪽 가운데 |
| `TIMED_SPIKE` 바닥 배수구 | 2 × 2 | 144 × 144 | 아래 가운데 |
